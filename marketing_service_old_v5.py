...
async def broadcast_campaign(campaign_id: str, segment_ids: List[str] = None, manual_emails: List[str] = None):
    ...
    db = get_db()
    _status_set_to_sending = False  # track so finally block knows if we need to clean up
    ...
    if has_existing_queue:
        # We are resuming. Fetch ONLY the contacts still marked as 'pending'.
        # Explicitly exclude 'sent' and 'failed' — never re-send to contacts who already received the email.
        ...
        if already_sent_count > 0:
            logger.info(f"Resume: skipping {already_sent_count} contacts already sent for campaign {campaign_id}.")
    ...
    for i in range(0, len(targets), batch_size):
      ...
      logger.info(f"Campaign {campaign_id} broadcast complete. Sent {sent_count}.")
        ...
        # Only mark failed if we actually started sending — don't overwrite paused/scheduled/quota-brake states
        try:
            ...
            if current_status == "sending":
                db.table("email_campaigns").update({
                    "status": "failed",
                    "total_sent": locals().get("sent_count", 0)
                }).eq("id", campaign_id).execute()
                logger.error(f"Campaign {campaign_id} marked as FAILED due to unhandled crash.")
