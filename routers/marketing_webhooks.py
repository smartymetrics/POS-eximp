...
async def resend_webhook(request: Request):
    ...
    if RESEND_WEBHOOK_SECRET:
        # Svix signs: "{svix-id}.{svix-timestamp}.{raw_body}" with HMAC-SHA256
        # The svix-signature header contains one or more "v1,<base64>" signatures (space-separated)
        svix_id        = request.headers.get("svix-id", "")
        svix_timestamp = request.headers.get("svix-timestamp", "")
        svix_signature = request.headers.get("svix-signature", "")

        if not svix_id or not svix_timestamp or not svix_signature:
            logger.warning("Missing svix-id / svix-timestamp / svix-signature header")
            raise HTTPException(status_code=401, detail="Missing signature headers")

        try:
            import base64
            # Build the signed content exactly as Svix does
            signed_content = f"{svix_id}.{svix_timestamp}.".encode() + raw_body

            # Secret is stored as "whsec_<base64>" — strip the prefix before decoding
            ...
            received_sigs = [s.split(",", 1)[1] for s in svix_signature.split(" ") if "," in s]

            if not any(hmac.compare_digest(expected_sig, sig) for sig in received_sigs):
                logger.warning("Invalid webhook signature")
                raise HTTPException(status_code=401, detail="Invalid signature")

            logger.info(f"Webhook signature verified for svix-id={svix_id}")
        ...
        logger.warning("RESEND_WEBHOOK_SECRET not set — skipping signature verification (unsafe for production).")
...
async def click_tracking(campaign_id: str, contact_id: str, url: str, request: Request):
    ...
    # Analytics/scoring writes must never block the redirect — the person clicked a real
    # link and is waiting for it to load. Any failure here is logged and swallowed.
    try:
        ...
        logger.error(f"Click-tracking analytics failed for campaign {campaign_id}/{contact_id}: {e}")
...
async def unsubscribe_one_click(token: str, request: Request):
    """
    RFC 8058 one-click unsubscribe. Gmail/Yahoo/Outlook's native 'Unsubscribe'
    button (driven by the List-Unsubscribe / List-Unsubscribe-Post headers) POSTs
    here directly with no cookies and no page rendering — it must succeed
    immediately with a 200, with no survey and no dependency on the click-tracking
