...
from email_service import send_payout_receipt_email, send_payout_claim_rejected_email, send_portal_invite_email, send_report_email, send_receipt_email
...
async def review_payout_request(request_id: str, data: PayoutReview, bg_tasks: BackgroundTasks, current_admin=Depends(require_roles(["admin", "super_admin"]))):
    ...
    if data.status == 'rejected':
        vendor = req.get('vendors')
        if vendor and vendor.get('email'):
            reason = data.rejection_reason or "Your claim did not pass the audit review. Please contact Finance for details."
            bg_tasks.add_task(
                send_payout_claim_rejected_email,
                req, vendor, reason, current_admin['sub']
            )
...
async def trigger_portal_invite(data: PortalInvite, bg_tasks: BackgroundTasks, current_admin=Depends(require_roles(["admin", "super_admin"]))):
    ...
    # Map frontend category values to the DB constraint-allowed values:
    #   portal_invites_category_check allows: 'company', 'contractor', 'partner', 'staff'
    #   The invite modal sends 'disbursement' for staff disbursements — remap it.
    CATEGORY_MAP = {
        "disbursement": "staff",
        "staff":        "staff",
        "company":      "company",
        ...
        "partner":      "partner",
        "individual":   "contractor",   # legacy alias
    }
    safe_category = CATEGORY_MAP.get(data.category, "staff")
