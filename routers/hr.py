...
async def get_expenses(current_admin: dict = Depends(verify_token)):
    """
    Returns all staff office-expenditure / reimbursement records from
    expenditure_requests.  We filter to payout_method IN ('reimbursement',
    'direct_pay') AND category IN office/reimbursement buckets so that
    commission claims don't bleed in here.
    """
    db = get_db()
    res = await db_execute(lambda: db.table("expenditure_requests")
        ...
        .in_("payout_method", ["reimbursement", "direct_pay"])
        .not_.in_("category", ["Sales Commission", "Partner Payout"])
        .order("created_at", desc=True)
        .execute()
    )
    rows = res.data if res.data else []
    # Normalise into a shape the frontend already understands
    normalised = []
    for r in rows:
...
async def update_expense(expense_id: str, request: Request, current_admin: dict = Depends(verify_token)):
    ...
    # Map frontend status strings to expenditure_requests canonical values
    status_map = {
        "Approved": "approved",
        "approved": "approved",
        "Rejected": "rejected",
    ...
    if new_status == "approved":
        ...
        update_payload["reviewed_at"] = __import__("datetime").datetime.utcnow().isoformat()
    if new_status == "paid":
        update_payload["paid_at"] = __import__("datetime").datetime.utcnow().isoformat()
    if data.get("hr_note"):
        update_payload["hr_note"] = data["hr_note"]
    if data.get("hr_verified") is not None:
        update_payload["hr_verified"] = bool(data["hr_verified"])
    ...
    if res.data:
        exp = res.data[0]
        # Notify staff via their vendor/requester link
