...
def migrate_bucket(bucket: str, dry_run: bool = False):
    logger.info(f"\n=== {bucket} ===")
    try:
    ...
    moved, skipped = 0, 0
    for path in paths:
        if cc.resource_exists(bucket, path):
            skipped += 1
            continue
    ...
    logger.info(f"{bucket}: {moved} migrated, {skipped} already on Cloudinary")
...
if __name__ == "__main__":
    ...
    for bucket in targets:
        moved, _ = migrate_bucket(bucket, dry_run=dry_run)
        total_moved += moved
