...
async def send_marketing_email(campaign: Dict[str, Any], contact: Dict[str, Any]):
    ...
    # Check if this contact has previously hard-bounced (failed sends)
    try:
        db = get_db()
        bounce_res = db.table("campaign_recipients").select("id").eq("contact_id", contact_id).eq("status", "failed").limit(1).execute()
        if bounce_res.data:
            logger.warning(f"Aborting send to {email} - Contact has a prior bounce record.")
            return None
    except Exception as bounce_err:
        logger.warning(f"Could not check bounce history for {email}: {bounce_err}")
    ...
    try:
        ...
        if campaign.get("status") != "test":
            db = get_db()
            # FIX: Use .update().eq() instead of .upsert() so we update the existing
            # 'pending' row rather than inserting a duplicate row (which caused
            # resending on resume and inflated safety-quota counts).
        ...
        if campaign.get("status") != "test":
            db = get_db()
            # FIX: Same here — update existing row rather than inserting a duplicate.
            db.table("campaign_recipients").update({
                "status": "failed",
                "sent_at": datetime.utcnow().isoformat()
            }).eq("campaign_id", campaign_id).eq("contact_id", contact_id).execute()
...
async def broadcast_campaign(campaign_id: str, segment_ids: List[str] = None, manual_emails: List[str] = None):
    ...
    # FIX: Tracks whether the loop exited via an intentional early return (pause / quota
    # brake) so the finally block knows NOT to overwrite that status.
    ...
    try:
      for i in range(0, len(targets), batch_size):
        ...
        if fresh.data and fresh.data[0]["status"] == "paused":
            ...
            _loop_completed = True  # FIX: mark so finally does NOT override 'paused'
        ...
        if quota["enabled"] and quota["remaining"] <= 0:
            ...
            _loop_completed = True  # FIX: mark so finally does NOT override 'scheduled'
            return # Exit broadcast

        # Calculate batch size based on remaining quota — re-fetch to get current value
        # This prevents overshoot when quota is near the limit
        ...
        if current_batch_size <= 0 and quota["enabled"]:
            # Quota just ran out — trigger the brake
            break
      ...
      # 5. Finalize — only reached if the loop ran to completion normally
      ...
      logger.info(f"Campaign {campaign_id} broadcast complete. Sent {sent_count}.")
        ...
        # FIX: Unhandled exception — mark as failed so it never stays stuck on 'sending'.
        logger.error(f"Campaign {campaign_id} broadcast crashed: {exc}", exc_info=True)
        try:
            db.table("email_campaigns").update({
                "status": "failed",
                "total_sent": sent_count,
            ...
            pass  # Best-effort; logging is the priority.
        raise
    finally:
        # FIX: Safety net — if the function exits without _loop_completed being set
        # (e.g. a 'return' from inside the try before _loop_completed=True), we check
        # the DB status and correct it if it is still stuck on 'sending'.
        if not _loop_completed:
            try:
                stuck = db.table("email_campaigns").select("status").eq("id", campaign_id).execute()
                if stuck.data and stuck.data[0]["status"] == "sending":
                    db.table("email_campaigns").update({
                        "status": "paused",
                        "total_sent": sent_count
                    }).eq("id", campaign_id).execute()
                    logger.warning(f"Campaign {campaign_id} exited early while 'sending'; set to 'paused'.")
            except Exception:
                pass
