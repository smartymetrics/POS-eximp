...
function ExpensesManager() {
  ...
  // Option A: use net_payout_amount, include partially_paid — matches payout dashboard owed figure
  const totalApproved = expenses.filter(e => ["approved", "partially_paid"].includes(normalize(e.status))).reduce((s, e) => s + parseFloat(e.net || e.amount || 0), 0);
  ...
  return (
    <div className="fade">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 22 }}>
        ...
        <StatCard label="Approved / Partial" value={expenses.filter(e => ["approved","partially_paid"].includes(normalize(e.status))).length} col="#4ADE80" sub={fmt(totalApproved)} />
