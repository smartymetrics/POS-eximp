...
function ExpensesManager() {
  ...
  const submitVerify = async () => {
    ...
    try {
      // All records now live in expenditure_requests — patch via /hr/expenses/{id}
  ...
  // ── Mark expense Paid — just patches the existing expenditure_request row ──
  const markPaid = async (exp) => {
    if (!window.confirm(`Mark ₦${parseFloat(exp.amount || 0).toLocaleString()} reimbursement for ${exp.staff?.full_name || "staff"} as Paid?`)) return;
    setPayingId(exp.id);
    try {
