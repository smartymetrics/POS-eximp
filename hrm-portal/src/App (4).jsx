...
function ExpensesManager() {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      const portal = (Array.isArray(pr) ? pr : []).map(r => ({
        ...
        status: r.status,
        receipt_url: r.proforma_url || null,
        remarks: r.remarks,
        source: "PORTAL",
        hr_verified: r.hr_verified || false,
        hr_note: r.hr_note || null,
        hr_verified_by: r.hr_verified_by || null,
      ...
      // ── Commission expenditure records from payout dashboard ──
      const commList = (Array.isArray(commReqs) ? commReqs : []).map(r => ({
        id: r.id,
        staff_id: r.vendors?.id,
        staff: { full_name: r.vendors?.name },
        invoice_number: r.metadata?.invoice_number || r.title,
        estate: r.metadata?.estate_name || r.category || "—",
        gross: r.amount_gross,
        wht: r.wht_amount || 0,
        net: r.amount_net || (r.amount_gross - (r.wht_amount || 0)),
        date: r.created_at,
        created_at: r.created_at,
        payout_status: r.status, // pending / approved / paid / rejected
        hr_verified: r.hr_verified || false,
        hr_note: r.hr_note || null,
        hr_verified_by: r.hr_verified_by || null,
        verified_at: r.verified_at || null,
  ...
  // ── HR quick approve/reject (no verification notes) ──
  ...
  const submitVerify = async () => {
    ...
    try {
      // Use the payout verify endpoint which logs the HR action — same as payout dashboard
      await apiFetch(`${API_BASE}/payouts/requests/${verifying.id}/verify`, {
        method: "PATCH",
        body: JSON.stringify({
          status: isApprove ? "approved" : "rejected",
          hr_verified: true,
          hr_note: hr_note || null,
  ...
  // ── Mark expense Paid & push to payout dashboard ──
  const markPaid = async (exp) => {
    if (!window.confirm(`Mark ₦${parseFloat(exp.amount || 0).toLocaleString()} reimbursement for ${exp.staff?.full_name || "staff"} as Paid and record in Payout Dashboard?`)) return;
    setPayingId(exp.id);
    try {
      ...
      await apiFetch(`${API_BASE}/payouts/requests`, {
        ...
        body: JSON.stringify({
          ...
          amount_gross: parseFloat(exp.amount || 0),
          currency: exp.currency || "NGN",
          category: exp.category,
          title: `Expense reimbursement: ${exp.description}`,
  ...
  const stCol = { Pending: T.gold, Approved: "#4ADE80", Rejected: "#F87171", Paid: "#60A5FA", approved: "#4ADE80", rejected: "#F87171", paid: "#60A5FA", pending: T.gold };
  ...
  const totalPending = expenses.filter(e => normalize(e.status) === "pending").reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  const totalApproved = expenses.filter(e => normalize(e.status) === "approved").reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  ...
  return (
    <div className="fade">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 22 }}>
        ...
        <StatCard label="Expense Claims Approved" value={expenses.filter(e => normalize(e.status) === "approved").length} col="#4ADE80" sub={fmt(totalApproved)} />
                    ...
                    {filteredExpenses.map(e => {
                      ...
                      return (
                        <tr key={e.id} style={{ cursor: "pointer" }} onClick={() => setDetail({ ...e, _kind: "expense" })}>
                          ...
                          <td style={{ fontSize: 12, color: C.sub, maxWidth: 180 }}>{e.description}</td>
                          ...
                          <td style={{ fontSize: 11, color: C.sub, maxWidth: 160 }}>{e.remarks ? <span title={e.remarks}>{e.remarks.length > 50 ? e.remarks.slice(0, 50) + "…" : e.remarks}</span> : <span style={{ color: C.muted }}>—</span>}</td>
                          ...
                          <td><span className="tg" style={{ background: `${sc}22`, color: sc, fontSize: 10 }}>{e.status}</span></td>
                          <td>
                            {e.hr_verified
                          ...
                          <td onClick={ev => ev.stopPropagation()}>
                            <div style={{ display: "flex", gap: 5 }}>
                              ...
                              {normalize(e.status) === "approved" && (
                                <button className="bp" style={{ fontSize: 10, padding: "4px 10px", background: "#10B981", opacity: payingId === e.id ? 0.6 : 1 }} disabled={payingId === e.id} onClick={() => markPaid(e)}>💳 Mark Paid</button>
                              )}
                  ...
                  <table className="ht">
                    <thead>
                      <tr>
                        ...
                        <th>WHT</th><th>Net Comm.</th><th>Date</th><th>Payout Status</th>
                    ...
                    <tbody>
                      ...
                      {filteredCommissions.length === 0 && (
                        <tr><td colSpan="10" style={{ textAlign: "center", padding: 40, color: C.muted }}>No commission records match the current filter.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
